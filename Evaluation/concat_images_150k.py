    ...
    def concat_images(self, pad=False, recurrent=False):
        ...
        if pad:
            ...
            for item in array_2d:
                array_2d[i] = np.pad(array_2d[i], ((0,0), (0,0), (0,0)), mode='constant', constant_values=255)
                i +=1
...
def make_image_list_sorted(image_dir, split_on):
    image_list_array = []
    
    directory = os.listdir(image_dir)
    sortedDir = sorted(directory,key=lambda x: int(x.split(split_on)[0]))
...
def concat_lists(image_list, directory_list, destination_dir, pad=False, orientation=1):
    ...
    for image_name in image_list:
        ...
        for i in range(1, len(directory_list)):
            print(directory_list[i]+image_name)
...
image_name = '150k'
source_image_dir = './candidate_real_embedded/150k_plus/image/'
destination_dir = './candidate_images_concat/'
image_list_dir = source_image_dir
split_on = '.'

image_list = make_image_list(source_image_dir)
directory_list = ['./candidate_real_embedded/150k_plus/image/',
                  './candidate_real_embedded/150k_plus/projection/',
                  './candidate_real_embedded/culverts_13000/projection/']
