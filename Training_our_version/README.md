...
python train.py --data 'images/preprocessed_512x512_no_culvert_mirror/' --outdir 'output/fine_tuned/' --snap 250 \
--resume './output/fine_tuned/00005--auto2-resumecustom/network-snapshot-008000.pkl' --gpus=2

exit
```
Similarly we have a data folder path and an output folder path. Here we define the frequency of log snapshots for our progress. We also define the .pkl file to resume training from. Again, we chose to use (2) gpus. 
